                ...
                summary_data.append(
                    {
                        ...
                        "ΔRMSE vs Benchmark (bps)": (best["RMSE_test"] - bench_rmse)
                        * 100,
...
def summarise_priced_in_regressions(
    ...
    for model_type, l1_ratio in model_configs:
        ...
        merged["Average"] = (
            ...
            .applymap(
                ...
                if isinstance(x, (int, float, str)) and str(x).strip("*") not in ["", "-"]
                else 0
            )
            .mean(axis=1)
            .round(2)
    ...
    cols = ["Average"] + [
        c
        for c in ["LASSO", "RIDGE", "EN (L1=0.25)", "EN (L1=0.5)", "EN (L1=0.75)"]
                    ...
                    all_rows.append(
                        {
                            ...
                            "H1=Equal predictive accuracy (DM test)": dm_result,
...
def summarise_temporal_signal_vars(specs_dir, base_name, models, all_labels):
    ...
    for h in sorted(sig_df["Horizon"].unique()):
        ...
        for var in all_vars:
            ...
            numbers = [
                v for v in entry.values() if isinstance(v, (int, float, np.number))
            ]
    ...
    pivot = (
        avg_df.pivot_table(
            values="Average", index="Label", columns="Horizon", aggfunc="mean", fill_value=0
        )
        .round(2)
        .sort_index(axis=1)
    )
