...
def show_simple_benchmark(df, split_share):
    ...
    axes[0].set_title(
        f"Mean-reversion benchmark ({split_label})\nRMSE={rmse:.2f}, R²={r2:.3f}"
    )
...
def summarise_signal_regressions(specs_dir, base_name, models, benchmark_dfs):
    for split_share, (split_tag, split_label) in {
        ...
        if benchmark_df is None or benchmark_df.empty:
            continue
        bench_r2 = benchmark_df.loc[
            benchmark_df["horizon"] == 1, "R2_test"
        ].values[0]
        bench_rmse = benchmark_df.loc[
            benchmark_df["horizon"] == 1, "RMSE_test"
        ].values[0]
        ...
        for model in models:
            for l1_ratio in [0.25, 0.5, 0.75] if model == "elastic" else [None]:
                ...
                reject = (
                    "Accepted"
                    if (pd.notna(best["DM_p"]) and best["DM_p"] < 0.05)
                    else "Rejected"
                )
                label = (
                    f"{model.capitalize()}"
                    if l1_ratio is None
                    else f"Elastic Net (L1={l1_ratio})"
                )
                summary_data.append(
                    {
                        "Model": label,
...
def summarise_priced_in_regressions(
    ...
    for model_type, l1_ratio in model_configs:
        ...
        for tag, slabel in split_tags.items():
            ...
            shap_df["val_str"] = shap_df.apply(
                lambda r: f"{r['shap_importance_pct']:.2f}{'*' if r['stable'] else ''}",
                axis=1,
            )
    ...
    final_df = (
        final_df[final_df["Average"] > 1][cols]
        .round(2)
            ...
            for l1_ratio in variants:
                for h in range(0, 11):
                    ...
                    if df.empty or "R²_test" not in df:
                        continue
                    best = df.loc[df["R²_test"].idxmax()]
                    bench_r2_h, bench_rmse_h = bench_r2.get(
                        h, np.nan
                    ), bench_rmse.get(h, np.nan)
                    r2_val, rmse_val, dm_stat, dm_p = (
                        best["R²_test"],
                        best["RMSE_test"],
                        best.get("DM_stat"),
                        best.get("DM_p"),
                    ...
                    if pd.notna(dm_p) and dm_p < 0.05:
                        if pd.notna(dm_stat) and dm_stat > 0:
                            dm_result = "Accepted (SIGNAL)"
                        elif pd.notna(dm_stat) and dm_stat < 0:
                            dm_result = "Accepted (NOISE)"
                        else:
                            dm_result = "Accepted"
                    else:
                        dm_result = "Rejected"
                    ...
                    if r2_val < 0:
                        ...
                        r2_disp = f"{r2_val:.4f}"
                        d_r2 = (
                            (r2_val - bench_r2_h) * 100
                            if pd.notna(bench_r2_h)
                            else np.nan
                        )
                        d_rmse = (
                            (rmse_val - bench_rmse_h) * 100
                            if pd.notna(bench_rmse_h)
                            else np.nan
                        )
...
def plot_temporal_signal_summary(
            ...
            for l1_ratio in variants:
                for h in range(0, 11):
                    ...
                    if df.empty or "R²_test" not in df:
                        continue
                    best = df.loc[df["R²_test"].idxmax()]
                    dm_stat, dm_p = best.get("DM_stat", np.nan), best.get(
                        "DM_p", np.nan
                    )
                    if pd.notna(dm_p) and dm_p < 0.05:
                        if pd.notna(dm_stat) and dm_stat > 0:
    ...
    summary = (
        df.groupby(["horizon", "outcome"]).size().reset_index(name="count")
    )
    colors = {
        "SIGNAL": "#2E86AB",
        "NOISE": "#E67E22",
    ...
    ax.set_ylabel("Count of DM outcomes")
    ax.set_title(
        "Forecast horizons — DM test outcome summary across all models and splits",
        fontsize=14,
    )
...
def summarise_temporal_signal_vars(specs_dir, base_name, models, all_labels):
            ...
            for model in models:
                ...
                for l1 in l1_vals:
                    if model == "elastic":
                        ...
                        base = (
                            f"{model}_{base_name}_t{h}{split_tag}_l1_{l1}_clust"
                        )
                    else:
    ...
    for h in sorted(sig_df["Horizon"].unique()):
        ...
        lasso_splits, ridge_splits, en_splits = (
            ...
            fmt(model_splits["Elastic Net (L1=0.5)"]),
        )
        rename_map = {
            "Lasso": f"Lasso {lasso_splits}",
            "Ridge": f"Ridge {ridge_splits}",
            "Elastic Net (L1=0.5)": f"Elastic Net (L1=0.5) {en_splits}",
        }
        model_cols = [
            "Average",
            f"Lasso {lasso_splits}",
    ...
    for h in horizons:
        ...
        for model_type in ["lasso", "ridge", "elastic"]:
            if model_type == "elastic":
                l1_ratio = 0.5
                fname = (
                    f"{model_type}_{prefix}_agn_l1_{l1_ratio}_clust_results.dat"
                )
...
def plot_temporal_stats(summary_df, benchmark_df, split_share):
    ...
    ax.set_title(
        f"Test R² vs forecast horizon ({split_share*100:.0f}-{(1 - split_share)*100:.0f} split)",
        fontweight="normal",
    )
    ...
    ax.set_title(
        f"Test RMSE vs forecast horizon ({split_share*100:.0f}-{(1 - split_share)*100:.0f} split)",
        fontweight="normal",
    )
    ax.grid(True, alpha=0.3)
    ax.spines["top"].set_visible(False)
...
def print_temporal_stats(summary_df, benchmark_df=None):
    ...
    pivot_rmse = (
        summary_df.pivot(index="horizon", columns="model", values="RMSE_test")
        .round(3)
        .copy()
    )
    rename_cols = {
